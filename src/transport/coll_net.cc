    ...
    for (int s=0; s<args->nsubs; s++) {
      ...
      if (LAST_OF_GROUP(s) && (sub->transmitted < sub->received)) {
        ...
        if (reqFifo[group][buffSlot].recvBuff != NULL) {
          ...
          TRACE(NCCL_NET,
                "sendProxy [%d/%d/%d] Iallreduce posted, size %d req %p",
                (int)sub->transmitted, group, buffSlot, totalSize,
      ...
      if (LAST_OF_GROUP(s) && sub->done < sub->transmitted) {
        ...
        if (done) {
          TRACE(NCCL_NET, "sendProxy [%d/%d/%d] request %p done, size %d",
                (int)sub->done, group, buffSlot, sub->requests[buffSlot], size);
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s=0; s<args->nsubs; s++) {
      ...
      if (LAST_OF_GROUP(s) && (sub->posted < sub->done + perGroupSteps) && (sub->posted < sub->nsteps)) {
        ...
        TRACE(NCCL_NET, "recvProxy [%d/%d/%d] posted buffer %p",
              (int)sub->posted, group, buffSlot,
              reqFifo[group][buffSlot].recvBuff);
        sub->posted += args->sliceSteps;
        args->idle = 0;
        continue;
      }
      if (LAST_OF_GROUP(s) && (sub->posted > sub->received)) {
        ...
        if (reqFifo[group][buffSlot].recvBuff == NULL) { // Buffer is cleared : coll is complete
          ...
          TRACE(NCCL_NET, "recvProxy [%d/%d/%d] received, size %d",
                (int)sub->received, group, buffSlot, totalSize);
          sub->received += args->sliceSteps;
      ...
      if (LAST_OF_GROUP(s) && (sub->received > sub->flushed)) {
        ...
        if (done) {
          TRACE(NCCL_NET, "recvProxy [%d/%d/%d] flushed", (int)sub->flushed,
                group, buffSlot);
          for (int i=group*COLLNET_GROUP_NSUBS; i<=s; i++) args->subs[i].flushed += args->sliceSteps;
          args->idle = 0;
          //continue;
